...
namespace KS.Foundation
{
	public class JSON
	{
		...
		protected static object ParseNumber(char[] json, ref int index, ref bool success)
		{
			...
			int lastIndex = GetLastIndexOfNumber(json, index, out isFloatingPoint);
			int charLength = (lastIndex - index) + 1;

			decimal d;
			success = Decimal.TryParse (new string (json, index, charLength), NumberStyles.Any, CultureInfo.InvariantCulture, out d);
			index = lastIndex + 1;
			return d;
		}			

		protected static int GetLastIndexOfNumber(char[] json, int index, out bool isFloatingPoint)
		{
			int lastIndex;
			...
			for (lastIndex = index; lastIndex < json.Length; lastIndex++) {
				...
				else if ("0123456789+-eE".IndexOf(json[lastIndex]) == -1) {
					break;
				}
			}
			return lastIndex - 1;
